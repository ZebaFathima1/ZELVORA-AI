...
import { Brain, Send, Sparkles, ArrowLeft, Plus, Loader2 } from "lucide-react";
import { API_BASE } from "@/lib/api";
import { useAuth } from "@/contexts/AuthContext";

const MODELS = [
  { id: "claude", label: "Claude Sonnet 4.5", color: "#A78BFA" },
  { id: "gpt", label: "GPT-5.2", color: "#10B981" },
  { id: "gemini", label: "Gemini 3 Flash", color: "#06B6D4" },
];
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        setMessages((m) => {
          const copy = [...m];
          copy[copy.length - 1] = { role: "assistant", content: "Hmm, something went wrong. Try again." };
          return copy;
        });
        setStreaming(false);
        return;
          ...
          try {
            ...
            if (obj.type === "session") setSessionId(obj.session_id);
              ...
              setMessages((m) => {
                ...
                copy[copy.length - 1] = {
                  role: "assistant",
                  content: copy[copy.length - 1].content + obj.content,
                };
                return copy;
              });
              ...
              setMessages((m) => {
                const copy = [...m];
                copy[copy.length - 1] = { role: "assistant", content: `Error: ${obj.message}` };
                return copy;
              });
            }
          } catch {}
      ...
      setMessages((m) => {
        const copy = [...m];
        copy[copy.length - 1] = { role: "assistant", content: "Network hiccup. Please try again." };
        return copy;
      });
    } finally {
      setStreaming(false);
          ...
          {messages.length === 0 && (
            <div className="text-center mt-10">
              ...
              <p className="mt-3 text-white/55 text-sm max-w-md mx-auto">
                Ask anything. Mindora will guide you to discover the answer — through stories, examples, and tiny experiments.
              </p>
            ...
            <AnimatePresence initial={false}>
              {messages.map((m, i) => (
                ...
                >
                  ...
                  >
                    <div className="text-sm leading-relaxed whitespace-pre-wrap">{m.content}
      ...
      <div className="border-t border-white/[0.06] bg-[#0A0A0F]/80 backdrop-blur-xl">
        <div className="max-w-3xl mx-auto px-6 py-4">
          <form
            ...
            className="flex items-center gap-3 glass-strong rounded-full pl-5 pr-2 py-2 focus-within:ring-1 focus-within:ring-indigo-500"
          >
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="Ask Mindora a question — like, why do leaves change color?"
          ...
          <div className="mt-2 text-center text-[10px] font-mono uppercase tracking-[0.18em] text-white/30">
            Mindora teaches — it doesn't give direct answers. Stay curious.
          </div>
        </div>
      </div>
    </div>
